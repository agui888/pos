...
public class PaginationInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
    	...
    	Object map = defaultParameterHandler.getParameterObject();
    	if (map instanceof Map) {
    		Map parameterMap = (Map)map;
            if (parameterMap != null) {
            	if (parameterMap.containsKey("_rulesql"))
            		rulesql = parameterMap.get("_rulesql");
            	
            	if (parameterMap.containsKey("_sort"))
            		sort = parameterMap.get("_sort");
            	
            	if (parameterMap.containsKey("_order"))
        ...
        if (rulesql != null) {
        	if (originalSql.toLowerCase().contains(" where ")) {
        		originalSql = originalSql + " and " + rulesql;        		
        	} else {
        		originalSql = originalSql + " where " + rulesql;
        	}
        }

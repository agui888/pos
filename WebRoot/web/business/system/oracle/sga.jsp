...
<html xmlns="http://www.w3.org/1999/xhtml"> 
  ...
  <body>
	<div style="width:98%;">
		...
		<form id="searchForm" name="searchForm" action="${path}/system/oracle_sga.action?menuid=${menuid}" method="post">
		</form>
		
		<div style="width: 100%; margin: 10px;" align="left">
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="GText">
			  <tr>
			    <td width="2%" valign="middle" align="center"><img src="${path}/global/images/base/sga1.gif" /></td>
			    <td valign="middle">Oracle SGA</td>
		...
		<table width="100%" border="0" cellpadding="2" cellspacing="1" bgcolor="#DDDDDD" class="BText" style="margin: 10px;">
		  <tr bgcolor="#F0F4F7">
		    ...
		    <td width="20%" align="center">值(bytes)</td>
		    <td width="20%" align="center">值(MB)</td>
	  	  </tr>
	  	  
	  	  <c:forEach items="${oracleSGA}" var="ls" varStatus="row">
			  <tr bgcolor='<c:if test="${row.index%2 eq 0}">#FFFFFF</c:if><c:if test="${row.index%2 ne 0}">#FBFDFF</c:if>' 
			    ...
			    <td align="right">${ls.BYTES} bytes</td>

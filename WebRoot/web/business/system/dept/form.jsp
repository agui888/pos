    ...
    <script>
        ...
        $(function() {
              grid = $("#cxGrid").treegrid({
                    ...
                    columns:[[
                              ...
                              {field:"deptname", title:"部门名称", sortable:true, width: <e:gridwidth list="${easyGrid}" gridtag="grid1" key="deptname" initial="350"/>},
                            	  ...
                            	  formatter:function(value, row) {
                                      <c:if test="${writePower ne true}">
                                          var str = "<div class='hidden-sm hidden-xs action-buttons'>" +
                                                         "<a class='green' href='#' onclick='ajaxModify(\"${menuid}\", \"${URI}\", \"" + row.id + "\", validForm , grid, $(\"#deptname\"), false)'>" +
        ...
        function ajaxBindForm(row) {
        	...
        	var data = {
                  	"deptEntity.id": row.id,
                  	"deptEntity.pid": row.pid,
                  	"deptEntity.deptname": row.deptname,
                  	"deptEntity.colid": row.colid
            };
           	return data;
        }
    </script>
                        ...
                        <table class="table-xs">
                            <tr>
                                ...
                                <td class="input-group-xs">
                                    <input name="deptEntity.deptname" class="form-control" size="50" type="text" onkeydown="if(event.keyCode==13){ajaxSearch();}"/>
                                </td>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="saveForm" method="post" action="${URI}_save.action?ajax=true&menuid=${menuid}" class="saveCalss form-horizontal" role="form">
                        <input type="hidden" name="deptEntity.id" />
                        <!-- 添加自己需要的 hidden 字段 -->
                            ...
                            <tr>
                                ...
                                <td width="53%" class="input-group-xs">
                                    <input class="easyui-combotree form-control" 
							               id="pid" 
							        	   name="deptEntity.pid" 
							        	   value="${deptEntity.pid}" 
							        	   style="width:300px;"
							        	   data-options="url:'${URI}_combobox.action?menuid=${menuid}', valueField:'id'"/>  <!-- 多选参数 multiple -->
                                </td>
                                <td>
                                    <span class="Validform_checktip">请选择上级部门</span>
                                </td>
                            </tr>
                            <tr>
                                <td height="30" align="right"><span class="RText">* </span>部门名称</td>
                                <td class="input-group-xs">
                                    <input class="form-control" type="text" id="deptname" name="deptEntity.deptname" style="width: 300px;" maxlength="50" sucmsg="当前部门名称验证通过!" nullmsg="请输入部门名称!" datatype="*1-50" errormsg="部门名称至少1个字符,最多50个字符!" ajaxurl=""  />
                                </td>
                                <td>
                                    <span class="Validform_checktip">部门名称为1~50个字符</span>
                                </td>
                            </tr>
                            <tr>
                                <td height="30" align="right"><span class="RText">* </span>序列</td>
                                <td class="input-group-xs">
                                    <input class="form-control" type="text" id="colid" name="deptEntity.colid" style="width: 300px;" maxlength="8" sucmsg="当前序列验证通过!" nullmsg="请输入序列!" datatype="*1-8" errormsg="序列至少1个字符,最多8个字符!"  />
                                </td>
                                <td>

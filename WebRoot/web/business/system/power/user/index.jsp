...
<html lang="en">
  <head>
	...
	<script>
		var zTreeObj;
	    var simpleNodes = ${userPowerMenuJosn};
		...
		function onCheck(e, treeId, treeNode) {
			...
			$.ajax({
                type: "post",
                url: "${path}/system/userpower_ispower.action?id=" + treeNode.userpowerid + "&value=" + value,
		...
		function beforeClick(treeId, treeNode) {
			if (treeNode.iframesrc == 'true') {
				document.getElementById("tableIframe").src = "${path}/web/business/system/power/user/welcome.jsp";
			} else {
				document.getElementById("tableIframe").src = "${path}/system/userpower_list.action?managepowerid=" + treeNode.managepowerid + "&userpowerid=" + treeNode.userpowerid + "&menuid=" + treeNode.menuid;
			}
		}
	 </script>
  </head>
  ...
  <body>
	<table width="100%">
	  <tr>
	    ...
	    <td valign="top">
			<iframe id="tableIframe" name="tableIframe" frameborder="0" height="436" src="${path}/web/business/system/power/user/welcome.jsp" scrolling="auto" width="100%"></iframe>			
	    </td>
	  </tr>
	</table>
